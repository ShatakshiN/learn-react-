...
export const seedCategories = async () => {
    ...
    for (const catData of topCategoriesData) {
        let existing = await categoryRepo.findOne({ where: { category: catData.category } });
        if (!existing) {
            const newCategory = categoryRepo.create({ ...catData, parent: null });
            existing = await categoryRepo.save(newCategory);
        }
        topCategories[catData.category] = existing;
    }
    ;
    ...
    for (const sub of subCategoriesData) {
        ...
        let existing = await categoryRepo.findOne({ where: { category: sub.category } });
        if (!existing) {
